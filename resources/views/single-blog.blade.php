...
@section('main_section')
    <style>
        ...
        .share-container {
            ...
            position: absolute;
            bottom: 50px;
            /* Adjust for positioning above button */
            left: 0;
            background: white;
        ...
        .btn-like {
            ...
            align-items: center;
            gap: 5px;
            /* Adjust spacing between heart and count */
            border: none;
            background: none;
            cursor: pointer;
            font-size: 20px;
    ...
    <style>
        ...
        .btn-share:hover {
            transform: scale(1.1);
            box-shadow: 0px 4px 8px rgba(16, 185, 129, 0.3);
        } */
...
<style>
    ...
    <div class="container mt-5">
        <div class="single-blog-container mb-5">
            <div class="row m-1 p-1 print-adjust" style="background: aliceblue">
                <div class="col-md-12">
                            ...
                            <div class="profile-details mt-3 mt-md-0 text-center text-md-start">
                                ...
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    {{-- <div class="d-flex align-items-center post-actions">
                                        ...
                                        <div>
                                            @if (auth()->check())
                                                <button class="action-btn btn-like" id="likeButton-{{ $blog->id }}" onclick="likePost({{ $blog->id }})">
                                                    ❤️ <span id="likeCount-{{ $blog->id }}">{{ $blog->likes->count() }}</span>
                                                    <span class="btn-label">Like</span>
                                                </button>
                                            @else
                                                <button class="action-btn btn-like" id="likeButton-{{ $blog->id }}" onclick="showLoginPrompt({{ $blog->id }})">
                                                    🤍 <span id="likeCount-{{ $blog->id }}">{{ $blog->likes->count() }}</span>
                                                    <span class="btn-label">Like</span>
                                                </button>
                                                <span id="loginPrompt-{{ $blog->id }}" class="text-danger d-block mt-1" style="display:none;">
                                        ...
                                        <!-- Comment Button -->
                                        <div>
                                            <button class="action-btn btn-comment" onclick="toggleComments()">
                                        ...
                                        <div class="position-relative">
                                            <button class="action-btn btn-share" onclick="toggleShareOptions()">
                                            ...
                                            <div id="shareLinks" class="share-container">
                                                <div class="share-links d-flex">
                                                    <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode($shareUrl) }}" target="_blank" onclick="updateShareCount({{ $blog->id }})">
                                                        <i class="fa-brands fa-facebook"></i>
                                                    </a>
                                                    <a href="https://twitter.com/intent/tweet?text={{ urlencode($shareTitle) }}&url={{ urlencode($shareUrl) }}" target="_blank" onclick="updateShareCount({{ $blog->id }})">
                                                        <i class="fa-brands fa-twitter"></i>
                                                    </a>
                                                    <a href="https://www.linkedin.com/sharing/share-offsite/?url={{ urlencode($shareUrl) }}" target="_blank" onclick="updateShareCount({{ $blog->id }})">
                                                        <i class="fa-brands fa-linkedin"></i>
                                                    </a>
                                                    <a href="https://api.whatsapp.com/send?text={{ urlencode($shareMessage . ' ' . $shareUrl) }}" target="_blank" onclick="updateShareCount({{ $blog->id }})">
                                                        <i class="fa-brands fa-whatsapp"></i>
                                                    </a>
                                                    <a href="https://telegram.me/share/url?url={{ urlencode($shareUrl) }}&text={{ urlencode($shareTitle) }}" target="_blank" onclick="updateShareCount({{ $blog->id }})">
                                                        <i class="fa-brands fa-telegram"></i>
                                                    </a>
                                                    <a href="mailto:?body={{ urlencode($shareMessage . ' ' . $shareUrl) }}" target="_blank" onclick="updateShareCount({{ $blog->id }})">
                                        ...
                                        <div class="views">
                                            <i class="fa fa-eye"></i>
                                            <span class="ms-1">{{ $blog->read_times }}</span>
                                            <span class="btn-label">Views</span>
                                        </div>
                                    </div> --}}
                                    ...
                                    <div class="d-flex align-items-center post-actions">
                                        ...
                                        <div>
                                            @if (auth()->check())
                                                ...
                                                <button class="like-btn" id="likeButton-{{ $blog->id }}" onclick="likePost({{ $blog->id }})">
                                                    <span class="like-count" id="likeCount-{{ $blog->id }}">{{ $blog->likes->count() }}</span>
                                                    <span class="heart">❤️</span>
                                                </button>
                                            @else
                                                <!-- Like Button for Logged-out Users -->
                                               {{--  <button class="action-btn btn-like" id="likeButton-{{ $blog->id }}" onclick="showLoginPrompt({{ $blog->id }})">
                                                    🤍 <span id="likeCount-{{ $blog->id }}">{{ $blog->likes->count() }}</span>
                                                    <span class="btn-label">Like</span>
                                                </button> --}}
                                        ...
                                        <div>
                                            {{-- <button class="action-btn btn-comment" onclick="toggleComments()">
                                                💬 <span id="commentCount">{{ $blog->comments()->count() }}</span>
                                                <span class="btn-label">Comment</span>
                                            </button> --}}
                                            <button class="btn-comment" onclick="toggleComments()">
                                                💬 
                                                <span id="commentCount-{{ $blog->id }}" class="comment-count">{{ $blog->comments()->count() }}</span>
                                            </button>
                                        </div>
                                        ...
                                        <div class="position-relative">
                                            ...
                                            <button class="btn-share" onclick="toggleShareOptions()">
                                                🔗 
                                                <span id="shareCount-{{ $blog->id }}" class="share-count">{{ $blog->share_count }}</span>
                                            </button>
                                        ...
                                        <div class="views">
                                            ...
                                            <button class="btn-views">
                                                <i class="fa fa-eye"></i>
                                                <span id="viewCount-{{ $blog->id }}" class="view-count">{{ $blog->read_times }}</span>
                                            </button>
                                        </div>
                                    </div>
                                </div>
                ...
                <div id="commentSection" style="border:none;">
                    <livewire:comments :model="$blog" />
                </div>
            </div>
        </div>
...
@section('customJs')
    ...
    <script>
        ...
        function likePost(postId) {
            if (likeCooldown[postId]) return; // Prevent spam clicking
    
            likeCooldown[postId] = true; // Set cooldown
            ...
            $.ajax({
                ...
                success: function(response) {
                    let likeCountElement = $('#likeCount-' + postId);
                    ...
                    if (response.message === 'Post liked') {
                        ...
                        // likeButton.html('❤️ <span id="likeCount-' + postId + '">' + response.likes + '</span> <span class="btn-label">Like</span>');
                        likeButton.addClass('liked'); 
                    } else {
                        likeButton.html('🤍 <span id="likeCount-' + postId + '">' + response.likes + '</span> ');
                        // likeButton.html('🤍 <span id="likeCount-' + postId + '">' + response.likes + '</span> <span class="btn-label">Like</span>');
                        likeButton.removeClass('liked');
                    }
    
                    setTimeout(() => likeButton.removeClass('liked-animation'), 300);
                },
                ...
                complete: function() {
                    setTimeout(() => { likeCooldown[postId] = false; }, 500);
                }
            });
        }
        ...
        function toggleShareOptions() {
            ...
            shareContainer.style.display = shareContainer.style.display === 'block' ? 'none' : 'block';
        }
        ...
        function updateCommentCount() {
            fetch(window.location.href) 
                ...
                .then(html => {
                    ...
                    let newCount = doc.querySelector('#commentCount').innerText;

                    document.getElementById('commentCount').innerText = newCount; // Update count
                })
                .catch(error => console.log("Error updating comment count: ", error));
        }
